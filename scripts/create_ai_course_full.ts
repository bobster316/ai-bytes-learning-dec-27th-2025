...
async function checkConnection() {
    ...
    if (error) {
        ...
        console.log(`Connection successful. 'courses' table has ${count} rows.`);
    }
}
checkConnection();
...
**Stage 1: Pre-Training** is like general education—elementary school through high school. This is where the AI reads enormous amounts of text from across the internet. We're talking about billions upon billions of words from books, websites, academic papers, code repositories, and more. During pre-training, the AI learns the fundamental patterns of language, absorbs factual knowledge about the world, and develops basic reasoning capabilities. This creates the foundation for everything that comes later.
...
The AI's internal parameters are updated based on the reward. Approaches that led to correct answers get strengthened.

**Step 6: Repeat—Millions of Times**
This process repeats with different problems, over and over.
...
async function seedCourse() {
    ...
    // 1. Get User via Admin API
    const { data: { users }, error: userError } = await supabase.auth.admin.listUsers();

    let userId: string | null = null;

    if (userError || !users || users.length === 0) {
        console.warn("⚠️ Could not fetch users via Admin API. Attempting fallback to 'profiles' table select...");
        // Fallback: Try selecting from profiles directly (often works if key has permissions)
        const { data: profiles } = await supabase.from('profiles').select('id').limit(1);
        if (profiles && profiles.length > 0) {
            userId = profiles[0].id;
        } else {
            console.error("❌ No user found via Admin API or Profiles table. Cannot assign course.");
            process.exit(1);
        }
    } else {
        userId = users[0].id;
    ...
    // 2. Create Course (Minimal Columns)
    ...
    if (!courses || courses.length === 0) {
        console.error("Course created but no data returned (RLS?).");
        process.exit(1);
    }
    ...
    for (let i = 0; i < COURSE_CONTENT.length; i++) {
        ...
        for (let j = 0; j < t.lessons.length; j++) {
            ...
            const contentJson = {
                ...
                topicType: 'foundation',
                keyTakeaway: "Understanding core AI training concepts.",
                humanVoiceSignature: "Professional Education",
                imagePrompts: l.imagePrompts.map((p, idx) => ({
                    prompt: p,
                    imageNumber: idx + 1,
                    imageType: "Instructional",
                    purpose: "Visual Aid",
                    technicalSpecs: "high quality"
                })),
                discussionPrompt: "How might these training methods apply to your field?",
                nextTopicTeaser: "Coming up next..."
            };
            ...
            const { data: lesson, error: lessonError } = await supabase
                ...
                .insert({
                    ...
                    title: l.title,
                    order_index: j,
                    content: contentJson, // Store as JSONB
                })
                .select()
                .single();
            ...
            if (lessonError) {
                ...
                const updatedContent = {
                    ...contentJson,
                    videoScript: {
                        hook: { duration: 5, script: l.videoScript.substring(0, 50), visualCues: [] },
                        coreContent: { segments: [{ script: l.videoScript, duration: 60, visualCues: [], codeSegments: [] }] },
                ...
                await supabase.from('course_lessons').update({ content: updatedContent }).eq('id', lesson.id);
            }
        }
    }
...
// seedCourse().catch(err => console.error("Fatal:", err));
...
(async () => {
    ...
    // Check Courses and Inspect Schema
    const { data: courses, error } = await supabase.from('courses').select('*').limit(1);

    if (error) {
        console.error("Courses check failed:", error);
    }

    else if (courses && courses.length > 0) {
        console.log(`✅ Columns: ${JSON.stringify(Object.keys(courses[0]))}`);
        // console.log("First row sample:", courses[0]);
    } else {
        console.log("Courses table is empty. Cannot inspect schema from data.");
    }
